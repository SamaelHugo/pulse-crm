...
function formatRelativeDate(dateStr: string): string {
  ...
  if (diffDays < 7) {
    ...
    const n1 = n % 10;
    const form =
      n > 10 && n < 20 ? "дней" : n1 > 1 && n1 < 5 ? "дня" : n1 === 1 ? "день" : "дней";
  ...
  const form = months === 1 ? "месяц" : months < 5 ? "месяца" : "месяцев";
  ...
  return (
    <div className="card-glow rounded-xl border border-border bg-bg-card">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              <th className="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-text-muted">Клиент</th>
              <th className="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-text-muted">Сделка</th>
              <th className="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-text-muted">Сумма</th>
              <th className="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-text-muted">Этап</th>
              <th className="px-6 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-text-muted">Дата</th>
            </tr>
          </thead>
          <tbody>
            {deals.map((deal) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <span className="text-sm text-text-secondary">{deal.title}</span>
                </td>
                ...
                <td className="px-6 py-4">
                  <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-medium ${stageStyles[deal.stage as DealStage] || ""}`}>
      ...
      <div className="border-t border-border px-6 py-3">
        <Link href="/deals" className="text-sm font-medium text-accent transition-colors duration-150 hover:text-accent-hover">
          Смотреть все →
        </Link>
      </div>
    </div>
